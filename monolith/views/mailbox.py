...
def see_sent_messages(): 
    if request.method == 'GET':
        msgs_sent = db.session.query(Message, User).filter(Message.id_receiver==User.id).filter(Message.id_sender==current_user.id).all()
...
def see_received_messages():
    if request.method == 'GET':
        msgs_rcv = db.session.query(Message, User).filter(Message.id_sender==User.id).filter(Message.id_receiver==current_user.id).all()
...
def see_draft_messages():
    if request.method == 'GET':
        draft_msgs = db.session.query(Message, User).filter(Message.id_receiver==User.id).filter(Message.id_sender==current_user.id).filter(Message.draft==True)
        print(draft_msgs)
